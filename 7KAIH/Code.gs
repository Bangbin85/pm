...
const SPREADSHEET_ID = "1KG-_0vlPkB1hb6JelrzLSDMYXxYPA8p8IRzXG6D_Qi8";
...
if (action === "login") { return handleLogin(data); }
  ...
  else if (action === "restoreAllData") { return handleRestoreAllData(data); } // <-- Tambahan baru
...
if (action === "readAll") { return handleReadAll(); }
  ...
  else if (action === "backupAllData") { return handleBackupAllData(); } // <-- Tambahan baru
...
// =================================================================================
// FUNGSI-FUNGSI HANDLER BACKUP & RESTORE (BARU)
// =================================================================================
...
function handleRestoreAllData(payload) {
  try {
    ...
    // Hapus semua sheet yang ada kecuali yang ada di backup untuk pembersihan total
    ...
    for (const sheetName of sheetNamesInBackup) {
      ...
      if (!sheet) {
        ...
        sheet.clear(); // Bersihkan sheet sebelum menimpa data
      }
...
settingsRange[12]
      ...
      waktu: {
        bangunPagi: settingsRange[20] ?
Utilities.formatDate(new Date(settingsRange[20]), scriptTimeZone, "HH:mm") : "05:00",
        tidurCepat: settingsRange[21] ?
Utilities.formatDate(new Date(settingsRange[21]), scriptTimeZone, "HH:mm") : "21:00"
      }
    };
...
const bangunPagiLimitStr = waktuSettings[0] ? Utilities.formatDate(new Date(waktuSettings[0]), scriptTimeZone, "HH:mm") : "05:00";
    const tidurCepatLimitStr = waktuSettings[1] ?
Utilities.formatDate(new Date(waktuSettings[1]), scriptTimeZone, "HH:mm") : "21:00";
...
if (rowData[11]) { try { const timeStr = Utilities.formatDate(new Date(rowData[11]), scriptTimeZone, "HH:mm"); if (timeStr <= tidurCepatLimitStr) totalScores.tidurCepat++;
} catch (e) {} }
        if (rowData[10]) { try { const timeStr = Utilities.formatDate(new Date(rowData[10]), scriptTimeZone, "HH:mm");
if (timeStr <= bangunPagiLimitStr) totalScores.bangunPagi++; } catch (e) {} }
...
for (const key in totalScores) {
          percentages[key] = (totalEntries > 0) ?
(totalScores[key] / totalEntries) * 100 : 0;
      }
...
function handleUpdate(data) {
    try {
        const sheet = data.type === "adminGuru" ?
sheetAdminGuru : sheetSiswa;
