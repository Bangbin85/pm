  ...
  try {
    ...
    for (const className of classNames) {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        let makanPoint = 0;
        if (String(rowData[15]).trim().toLowerCase() === 'ya') makanPoint++;
        if (String(rowData[16]).trim().toLowerCase() === 'ya') makanPoint++;
        if (String(rowData[17]).trim().toLowerCase() === 'ya') makanPoint++;
