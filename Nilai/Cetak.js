...
function getStatusKeputusanText(status, info) {
    ...
    if (isKelasAkhir) {
        ...
        if (status.toLowerCase().includes('naik') && !status.toLowerCase().includes('tidak')) {
            ...
            if (info.kelas) {
                if (info.kelas.includes('I') || info.kelas.includes('V') || info.kelas.toUpperCase().includes('X')) {
                    if (currentNum === 1) textKelasBaru = info.kelas.replace('I', 'II');
                    else if (currentNum === 2) textKelasBaru = info.kelas.replace('II', 'III');
                    else if (currentNum === 3) textKelasBaru = info.kelas.replace('III', 'IV');
                    else if (currentNum === 4) textKelasBaru = info.kelas.replace('IV', 'V');
                    else if (currentNum === 5) textKelasBaru = info.kelas.replace('V', 'VI');
                    else if (currentNum === 7) textKelasBaru = info.kelas.toUpperCase().replace('VII', 'VIII');
                    else if (currentNum === 8) textKelasBaru = info.kelas.toUpperCase().replace('VIII', 'IX');
                    else if (currentNum === 10) textKelasBaru = info.kelas.toUpperCase().replace('X', 'XI');
                    else if (currentNum === 11) textKelasBaru = info.kelas.toUpperCase().replace('XI', 'XII');
                } else {
                    textKelasBaru = info.kelas.replace(String(currentNum), String(nextNum));
                }
            }
            return `NAIK KELAS KE ${textKelasBaru.toUpperCase()}`;
        } else {
            return `TIDAK NAIK KELAS (MENGULANG DI KELAS ${info.kelas ? info.kelas.toUpperCase() : currentNum})`;
        }
    }
}
        ...
        <div id="nilaiModalSPA" class="modal-backdrop">
            <div class="cetak-modal-wrapper">
                ...
                <div class="cetak-modal-body" id="nilaiModalBody">
                    ...
                    <div id="modalKokurikuler" style="border: 1px solid black; padding: 10px; min-height: 40px; text-align:justify; white-space:normal;"></div>
                    <br>
                    <table>
...
window.showNilaiModal = function(siswaId) {
    ...
    document.getElementById('modalKokurikuler').innerHTML = Object.values(dataRapor.deskripsiKokurikuler?.[siswa.id] || {}).join('<br>') || '-';
...
function getNilaiPdfContent(siswaId) {
    ...
    // 2. Setup Kokurikuler
    const kokuText = Object.values(dataRapor.deskripsiKokurikuler?.[siswa.id] || {}).join('\n') || '-';
